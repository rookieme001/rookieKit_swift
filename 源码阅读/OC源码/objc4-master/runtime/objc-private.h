...
#   define ISA_MAGIC_VALUE 0x000001a000000001ULL
    struct {
        uintptr_t indexed           : 1; // 判断是否为纯指针型 （0：纯ISA指针型，代表当前对象类对象地址 1：非针型包ISA指，含内存管理方面数据） 后面分析基于非针型包ISA指
        uintptr_t has_assoc         : 1; // 是否有动态绑定属性
        uintptr_t has_cxx_dtor      : 1; // 是否包含有c++代码,ARC中（表示有些内存对象使用isa管理）
        uintptr_t shiftcls          : 33; // MACH_VM_MAX_ADDRESS 0x1000000000 /** 当前对象类对象地址 */
        uintptr_t magic             : 6;  // 用于对未初始化的对象进行校验
        uintptr_t weakly_referenced : 1;  // 是否包含弱应用
        uintptr_t deallocating      : 1;  // 是否正在执行dealloc操作
        uintptr_t has_sidetable_rc  : 1;  // 是否外挂引用计数表
        uintptr_t extra_rc          : 19; // 引用计数存储 （当isa引用计数少使用该方案）
